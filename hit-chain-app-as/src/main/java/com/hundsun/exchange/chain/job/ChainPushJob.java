...
@QuartzJobRegister(cronExpression = "0 0/5 * * * ?", group = "chain_group", config = "cron.chain.push")
public class ChainPushJob extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext jobExecutionContext) {
        log.info("仓储仓单上链数据推送定时任务开始");
        chainController.push();
        log.info("仓储仓单上链数据推送定时任务结束");
    }
}